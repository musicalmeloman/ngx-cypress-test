export class SmartTable {
    ...
    addNewRecordWithFirstandLastName (firsName, lastName) {
        cy.get('thead').find('.nb-plus').click()
           cy.get('thead').find('tr').eq(2).then( tableRow => {
               cy.wrap(tableRow).find('[placeholder="First Name"]').type(firsName)
           ...
           cy.get('tbody').find('tr').eq(0).then( tableRowUpdated => {
               cy.wrap(tableRowUpdated).find('td').eq(2).should('contain', firsName)
               cy.wrap(tableRowUpdated).find('td').eq(3).should('contain', lastName)
           })
    }
